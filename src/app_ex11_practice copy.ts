...
class ThreeJSContainer {
    ...
    private createScene = () => {
        ...
        // Tweenでコントロールする変数の定義
        let tweeninfo = {scaleX: 1.0,scaleY: 1.0};

        //  Tweenでパラメータの更新の際に呼び出される関数
        let updateScale =()=>{
            cube.scale.x = tweeninfo.scaleX;
            cube.scale.y = tweeninfo.scaleY;
        }

        // Tweenの作成
        const tween = new TWEEN.Tween(tweeninfo).to({ scaleX: 5 }, 1000).easing(TWEEN.Easing.Elastic.Out).onUpdate(updateScale);
