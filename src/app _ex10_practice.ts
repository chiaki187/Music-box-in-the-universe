...
class ThreeJSContainer {
    ...
    private createScene = () => {
        ...
        let lerp = (p0: THREE.Vector3, p1: THREE.Vector3,p2: THREE.Vector3,p3: THREE.Vector3, t: number) : (THREE.Vector3) => {
            ...
            const result2= p0.multiplyScalar((1-t)*(1-t)*(1-t)).add(p1.multiplyScalar(3*t*(1-t)*(1-t))).add(p2.multiplyScalar(3*t*t*(1-t))).add (p3.multiplyScalar(t*t*t))   ;                                    
            return result2;
        }
        ...
        let hermite = (p0: THREE.Vector3, v0: THREE.Vector3, 
            ...
            const result =p0.multiplyScalar((2*t+1)*(1-t)*(1-t)).add(v0.multiplyScalar(t*(1-t)*(1-t))).add(p1.multiplyScalar(t*t*(3-2*t))).add(v1.multiplyScalar(-t*t*(1-t))) //エルミート曲線を実装する
            return result;   
        }
        ...
        points2.push(new THREE.Vector3(0, 0, 8));
        ...
        let update: FrameRequestCallback = (time) => {
            ...
            const pos = hermite(points2[0], points2[1],points2[2],points2[3], t);
