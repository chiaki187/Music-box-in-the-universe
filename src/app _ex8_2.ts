...
class ThreeJSContainer {
    ...
    ///--------------------------------
     ...
     let addSceneFromObjFile = async (filePath: string,materialPath: string) => {  
            ...
            //マテリアル
            for(let i=0;i< meshLines.length;i++){
                const mateLine=mateLines[i];
                const mateSpaceSplitArray=mateLine.split(" ");
                const mateType=mateSpaceSplitArray[0];

                if(mateType=="Kd"){
                    verColor.push(parseFloat(mateSpaceSplitArray[1]));
                    verColor.push(parseFloat(mateSpaceSplitArray[2]));
                    verColor.push(parseFloat(mateSpaceSplitArray[3]));
            ...
            const material = new THREE.MeshBasicMaterial( { color: new THREE.Color(verColor[0], verColor[1], verColor[2]) }  );
            
    
            const mesh = new THREE.Mesh( geometry, material );
            this.scene.add(mesh);
        }
    addSceneFromObjFile("tri_mat.obj","tri_mat.mtl");    
